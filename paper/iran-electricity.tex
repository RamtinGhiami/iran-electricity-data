...
A further limit of the trend is what it cannot see. The projection carries no information about the programmes that would bend either line, because project-level data on them are not in the snapshot: plant efficiency upgrades (open-cycle to combined-cycle conversion, i.e.\ the loss in turning fuel into electricity), distribution loss-reduction programmes, capacity under construction with commissioning dates, and the pace of renewable additions. Each exists in official documents identified in the provenance register (the generation volume of the detailed statistics, the projects pipeline, the renewable authority's figures). The numbers in Table~ef{tab:forecast} are therefore the baseline those programmes have to beat, not a prediction that they fail.
